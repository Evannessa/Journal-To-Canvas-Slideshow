...
function findBoundingTile(sceneTiles) {
  ...
  for (let element of tiles) {
    if (
      element.getFlag("journal-to-canvas-slideshow", "name") == "boundingTile"
    ) {
...
async function displayImageInScene(ev, externalURL) {
  ...
  if (!boundingTile) {
    ...
    imageUpdate = await scaleToBoundingTile(
      displayTile,
      boundingTile,
      tex,
      url
...
async function createBoundingTile() {
  ...
  console.log(boundingTile);

  boundingTile[0].setFlag(
    "journal-to-canvas-slideshow",
    "name",
    "boundingTile"
  );
...
function applySceneHeaderButtons(app, html, options) {
  ...
  if (
    game.settings.get("journal-to-canvas-slideshow", "hideHeaderToggle") ==
    "show"
  ) {
...
Hooks.on("getSceneControlButtons", (controls) => {
  ...
  if (game.user.isGM) {
    if (
      game.settings.get("journal-to-canvas-slideshow", "hideTileButtons") ==
      true
    ) {
...
function applyClasses(app, html) {
  if (game.user.isGM) {
    ...
    if (app.actor != undefined) {
      ...
      if (
        game.settings.get("journal-to-canvas-slideshow", "useActorSheetImages")
      ) {
        const imgs = html
          .find("img[data-edit]")
...
Hooks.once("ready", () => {
  ...
  if (
    game.settings.get("journal-to-canvas-slideshow", "showWelcomeMessage") ==
      true &&
    game.user.isGM
  ) {
