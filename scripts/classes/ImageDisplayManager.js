...
export class ImageDisplayManager {
    static async getTilesFromArtScene() {
        ...
        let artTiles = await game.JTCS.tileUtils.getSceneSlideshowTiles(
            'art',
            true,
            {
        ...
        let frameTiles = await game.JTCS.tileUtils.getSceneSlideshowTiles(
            'frame',
            true,
            {
                currentSceneID: artSceneID
    ...
    static async updateTileObjectTexture(
        ...
        let updated = await ourScene.updateEmbeddedDocuments('Tile', [imageUpdate])
        // .catch((error)=> {
        //     console.log(error)
        //     ui.notifications.error(
        ...
        // );
        let obj = {
            "src": "modules/journal-to-canvas-slideshow/demo-images/pd42-63143351-rob.webp",
            "anchorX": 0.5,
            "anchorY": 0.5,
            "offsetX": 0,
            "offsetY": 0,
     ...
     * @param {*} url
     */
    static async determineDisplayMethod(
        sheetImageData = { method: 'window', url: '' }
    ) {
    ...
    static getImageSource(imageElement) {
        ...
        if (type == 'IMG') {
            ...
            type == 'DIV' &&
            imageElement.classList.contains('lightbox-image')
        ) {
